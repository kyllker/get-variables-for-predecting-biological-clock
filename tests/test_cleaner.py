...
class TestCleaner:
    ...
    def test__checker_filter_valid_columns(self):
        df2 = self.df.copy()
        df2['Id'] = [1, 1]
        dataframe_desired_columns = self.cleaner_object.filter_desired_columns(df2, ['A'], 'Id')
        assert dataframe_desired_columns.equals(pd.DataFrame([[1], [1]], columns=['A']))

    def test__checker_filter_not_valid_columns(self):
        df2 = self.df.copy()
        df2['Id'] = [1, 1]
        dataframe_desired_columns = self.cleaner_object.filter_desired_columns(df2, ['C'], 'Id')
        assert dataframe_desired_columns.equals(df2)

    def test__remove_duplicate_columns(self):
        df2 = self.df.copy()
        df2['C'] = [1, 1]
    ...
    def test__remove_duplicate_columns_when_there_are_not(self):
        df2 = self.df.copy()
        dataframe_drop_duplicate_columns = self.cleaner_object.remove_duplicate_columns(df2)
        assert self.df.equals(dataframe_drop_duplicate_columns)

    def test__remove_constant_columns(self):
        df2 = self.df.copy()
        df2['C'] = [1, 3]
    ...
    def test__remove_constant_columns_when_there_are_not(self):
    ...
    def test__convert_to_numerical_values_column_with_more_than_two_different_values(self):
        test_df = pd.DataFrame([[1, 'a'], [2, 'b'], [3, 'c']], columns=['A', 'B'])
        result_df = self.cleaner_object.convert_to_numerical_values_column_with_more_than_two_different_values(test_df)
        true_df = pd.DataFrame([[1, 1, 0, 0], [2, 0, 1, 0], [3, 0, 0, 1]], columns=['A', 'B_a', 'B_b', 'B_c'])
        assert result_df.equals(true_df)
    ...
    def test__predict(self):
        ...
        df_res = self.cleaner_object.predict(df, ['ID_Muestra', 'A', 'B', 'C', 'D'], 'ID_Muestra')
