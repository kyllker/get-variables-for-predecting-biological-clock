...
class TestInputer:
    def setup_class(self):
        self.seed = 42
        self.inputer_object = Inputer(self.seed)
    ...
    def test_build_an_imputer_object(self):
        assert isinstance(self.inputer_object, Inputer)

    def test_check_there_are_na_values_false(self):
        assert not self.inputer_object.check_there_are_na_values(self.df)

    def test_check_there_are_na_values_true(self):
        assert self.inputer_object.check_there_are_na_values(self.df_na)

    def test_get_na_and_no_na_columns(self):
        assert self.inputer_object.get_na_and_no_na_columns(self.df_na)[0][0] == 'B' and \
               self.inputer_object.get_na_and_no_na_columns(self.df_na)[1][0] == 'A'

    def test_knn_classifier(self):
        X_train = self.iris.data[:145]
        y_train = self.iris.target[:145]
        X_predict = self.iris.data[145:]
        assert all(self.inputer_object.knn_classifier(X_train, y_train, X_predict) == [2, 2, 2, 2, 2])

    def test_knn_regressor(self):
        X_train = self.iris.data[:145]
        y_train = self.iris.target[:145]
        X_predict = self.iris.data[145:]
        assert all(self.inputer_object.knn_regressor(X_train, y_train, X_predict) == [2.0,  1.6, 2.0,  2.0,  1.8])

    def test__imputer_normalize_dataframe(self):
        df2 = pd.DataFrame([[1, 1], [2, 3]], columns=['B', 'C'])
        dataframe_drop_constant_columns = self.inputer_object.normalize_dataframe(df2)
    ...
    def test_predict_with_knn_regressor(self):
        ...
        df_iris_no_na = self.inputer_object.predict(df_iris, 'knn')
